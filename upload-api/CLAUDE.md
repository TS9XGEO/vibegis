...
| `POST /upload-raster-zip` | `require_tier("pro")` | zip of single-band rasters (e.g. Sentinel-2) → every readable band published immediately as its own layer, no picker. `title` names the batch, not a band; each band's name comes from `band_label()` (GDAL band description, then common metadata keys, then the file name), which `/upload-raster` also uses when no `title` is given |
...
| `GET /column-breaks` | `require_login` | k+1 class edges for a graduated classification — `method=equal\|quantile\|jenks`, `classes=2..12`. Quantile is `percentile_cont` (exact); Jenks is the exact Fisher-Jenks DP on an even-stride sample (`JENKS_SAMPLE_CAP` 1200, ~90 ms at k=12 over 98k rows). Server-side because both describe the *distribution* — a client-side sample would answer differently on a big layer than a small one |
| `GET /column-groupby` | `require_tier("pro")` | value+count per distinct value, capped like `/distinct-values`, plus an exact `totalCount` — the dashboard overview's server-side group-by |
...
| `GET /healthz` | none | unauthenticated liveness for the Docker healthcheck. Deliberately has **no** nginx location — container-internal only, unreachable off-host |
...
| `GET /auth/superset` | `require_login` | nginx's `auth_request` target for `/analytics`: 200 + `X-Vibegis-User`/`-Role`, forwarded to Superset as `X-Remote-User`. Separate from `/auth/verify`, which authorizes named layers and so fails closed on every Superset URL |
| `GET /internal/superset/acl` | `require_internal_token` | role, tier and the `(schema, table)` pairs behind this user's visible layers. Exists so `visible_layers_for()` stays the ACL's only implementation — Superset asks rather than re-deriving |
